<?php
// project-root/admin/rekap_bisyaroh_guru.php
...
// Inisialisasi model dan ambil nominal gaji per jam
$nominal_gaji_per_pertemuan = 0; // Ganti nama variabel
if ($pdo) {
    try {
        $gaji_settings_model = new GajiSettings($pdo);
        $gaji_settings = $gaji_settings_model->getSettings();
        if ($gaji_settings && isset($gaji_settings['nominal_per_pertemuan'])) { // Perhatikan perubahan nama kolom di sini
...
function getGuruMonthlyTeachingData($month, $academic_year_str) {
    ...
    // Menambahkan `jp.jumlah_jam` dan `jp.jam_mulai`, `jp.jam_selesai` dari jadwal_pelajaran
    $sql = "
        SELECT
            g.id AS id_guru,
            g.nama_lengkap,
...
// Fungsi calculateGuruMonthlyHoursInfo dipertahankan jika ingin menampilkan total jam mengajar sebagai informasi
    ...
    if (empty($available_academic_years)) {
        ...
        foreach ($guru_teaching_data as $entry) {
            ...
            if (!isset($rekap_data[$guru_id])) {
                $rekap_data[$guru_id] = [
                    'nama_guru' => $nama_guru,
                    'jumlah_kehadiran' => 0, // Ganti nama variabel
                    'total_jam_dibayar' => 0, // Variabel baru untuk total jam yang dibayar
                    'total_gaji' => 0.0, // Total gaji berdasarkan jam mengajar
            ...
            $rekap_data[$guru_id]['total_jam_dibayar'] += $jumlah_jam_jadwal; // Tambahkan jumlah jam dari jadwal
        ...
        // Hitung total gaji berdasarkan total jam dibayar dan nominal per jam
        foreach ($rekap_data as $guru_id => $data) {
            $rekap_data[$guru_id]['total_gaji'] = $data['total_jam_dibayar'] * $nominal_gaji_per_pertemuan;

            // Ambil informasi total jam mengajar dari hasil calculateGuruMonthlyHoursInfo
            // Jika Anda ingin menampilkan jam aktual, bukan jam dari jadwal, Anda bisa menggunakan ini.
            // Saat ini, saya menghapusnya dari tampilan utama untuk menghindari kebingungan.
            // Jika ingin ditampilkan, Anda perlu memanggil fungsi ini:
            // $guru_monthly_hours_info = calculateGuruMonthlyHoursInfo($guru_teaching_data);
        ...
        if (isset($_POST['export_excel']) && !empty($rekap_data)) {
            ...
            try {
                ...
                $sheet->setCellValue('C4', 'Total Jam Mengajar'); // Ganti teks
                $sheet->setCellValue('D4', 'Total Gaji (Rp)');
                ...
                foreach ($rekap_data as $data) {
                    ...
                    $sheet->setCellValue('C' . $row_num, $data['total_jam_dibayar'] . ' jam'); // Tampilkan total jam yang dibayar
                ...
                $filename = "rekap_gaji_guru_" . str_replace('/', '-', $selected_academic_year_str) . "_Bulan_" . $months[$selected_month] . ".xlsx";
...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
    <h1 class="text-3xl font-semibold text-gray-800 mb-4 sm:mb-0">Rekap Bisyaroh Guru</h1>
</div>
