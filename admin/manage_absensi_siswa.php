<?php
// project-root/admin/manage_absensi_siswa.php
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'record_absensi') {
        ...
        $kelas_id_siswa = (int)($_POST['kelas_id_siswa'] ?? null); // Dapatkan kelas_id dari form
        ...
        if (empty($siswa_id) || empty($status_absensi) || !in_array($status_absensi, $status_kehadiran_options)) {
            ...
            // Determine the type of attendance to record
            if ($status_absensi === 'Pulang') {
                $result = $absensi_harian_model->recordSiswaAttendanceWithType(
                    $siswa_id,
                    'pulang', // Pass 'pulang' as the type
                    $selected_tahun_ajaran,
                    $selected_semester,
                    $kelas_id_siswa,
                    null // No manual status needed for 'pulang'
                );
            } else {
                $result = $absensi_harian_model->recordSiswaAttendanceWithType(
                    $siswa_id,
                    'manual', // Pass 'manual' as the type
                    $selected_tahun_ajaran,
                    $selected_semester,
                    $kelas_id_siswa,
                    $status_absensi // Pass the selected manual status
                );
            }
            ...
            if ($result['status'] === 'success') {
                ...
                $siswa_info = $siswa_model->findById($siswa_id); // Ambil info siswa untuk notifikasi
                if ($siswa_info) {
                    $phone_number_ortu = $siswa_info['no_telp_ortu'] ?? null;
                    if ($phone_number_ortu) {
                        $phone_number_clean = ltrim($phone_number_ortu, '+');
                        $nama_siswa = $siswa_info['nama_lengkap'];
                        $waktu_absensi_manual = date('H:i'); // Waktu saat admin mencatat

                        // Tentukan template dan parameter berdasarkan status absensi
                        //$template_name = 'absensi_notifikasi_manual_siswa'; // Template baru untuk notifikasi manual siswa
                        $template_params = [
                            $nama_siswa,
                            $status_absensi,
                            $tanggal_absensi,
                        ...
                        if ($queue_result['status'] === 'success') {
                            error_log("Notifikasi manual siswa " . $nama_siswa . " dengan status " . $status_absensi . " ditambahkan ke antrean.");
                        } else {
                            error_log("Gagal menambahkan notifikasi manual siswa " . $nama_siswa . " ke antrean: " . $queue_result['message']);
                        }
                    } else {
                        error_log("Nomor telepon orang tua siswa " . $siswa_info['nama_lengkap'] . " tidak ditemukan untuk notifikasi WhatsApp.");
                    }
                }
        ...
        redirect(ROOT_URL . 'admin/manage_absensi_siswa.php?kelas_id=' . urlencode($selected_kelas_id ?? '') . '&tanggal=' . urlencode($tanggal_absensi) . '&tahun_ajaran=' . urlencode($selected_tahun_ajaran) . '&semester=' . urlencode($selected_semester));
    }
                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-50">
                                ...
                                <td class="py-3 px-6 text-center whitespace-nowrap">
                                    <form action="" method="POST" class="inline-block">
                                        <!-- TODO: Ganti konfirmasi JavaScript dengan modal kustom jika diperlukan -->
                                        <input type="hidden" name="action" value="record_absensi">
                                        <input type="hidden" name="siswa_id" value="<?php echo htmlspecialchars($siswa['id']); ?>">
                                        <input type="hidden" name="tanggal_absensi" value="<?php echo htmlspecialchars($selected_tanggal); ?>">
                                        <input type="hidden" name="kelas_id_siswa" value="<?php echo htmlspecialchars($siswa['kelas_id']); ?>">
