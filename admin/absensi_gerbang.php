<?php
// project-root/admin/absensi_gerbang.php
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['is_ajax_request'])) { // Add a flag to distinguish AJAX
        ...
        if (empty($rfid_tag) || empty($tipe_absensi) || empty($jenis_pengguna)) {
            ...
            redirect(ROOT_URL . 'admin/absensi_gerbang.php');
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const qrCodeSuccessCallback = async (decodedText, decodedResult) => {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                resultDiv.textContent += `\nNama: ${data.nama}, Waktu: ${data.waktu}, Tipe: ${data.user_type}, Status: ${data.type === 'masuk' ? 'Masuk' : (data.type === 'pulang' ? 'Pulang' : 'Sudah Absen')}`;
    ...
    function displayManualResult(data) {
        ...
        document.getElementById('manual-result-status').textContent = data.type === 'masuk' ? 'Masuk' : (data.type === 'pulang' ? 'Pulang' : 'Sudah Absen');
