...
if (isset($_POST['action']) && $_POST['action'] === 'view_report') {
    if (!empty($selected_kelas_id)) {
        ...
        redirect(ROOT_URL . 'admin/rekap_absensi_mapel.php');
    }
}
...
if (isset($_POST['action']) && $_POST['action'] === 'send_weekly_summary_mapel') {
    ...
    redirect(ROOT_URL . 'admin/rekap_absensi_mapel.php?kelas_id=' . urlencode($selected_kelas_id ?? '') . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate));
}
