...
try {
    ...
    if (!$settings) {
        $settings = [
            ...
            'lokasi_sekolah_lat' => '', // NEW
            'lokasi_sekolah_long' => '', // NEW
            'radius_absensi_meter' => 50, // NEW
            'qr_code_path' => null, // NEW
            'qr_token_hash' => null
        ];
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $data = [
            ...
            'lokasi_sekolah_lat' => trim($_POST['lokasi_sekolah_lat'] ?? ''), // NEW
            'lokasi_sekolah_long' => trim($_POST['lokasi_sekolah_long'] ?? ''), // NEW
            'radius_absensi_meter' => (int)($_POST['radius_absensi_meter'] ?? 50), // NEW
            'qr_code_path' => $settings['qr_code_path'], // Keep existing QR code path
            'qr_token_hash' => $settings['qr_token_hash'] // Keep existing QR code path
        ];
...
<div class="container mx-auto p-4">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <h2 class="text-xl font-semibold mb-4">Pengaturan Absensi QR Code</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    <label for="lokasi_sekolah_lat" class="block text-gray-700 text-sm font-bold mb-2">Latitude Sekolah:</label>
                    <input type="text" id="lokasi_sekolah_lat" name="lokasi_sekolah_lat" value="<?php echo htmlspecialchars($settings['lokasi_sekolah_lat'] ?? ''); ?>" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
                <div>
                    <label for="lokasi_sekolah_long" class="block text-gray-700 text-sm font-bold mb-2">Longitude Sekolah:</label>
                    <input type="text" id="lokasi_sekolah_long" name="lokasi_sekolah_long" value="<?php echo htmlspecialchars($settings['lokasi_sekolah_long'] ?? ''); ?>" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
                <div class="col-span-2 mt-2">
                    <button type="button" id="getLocationBtn" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline">
                        <i class="fas fa-map-marker-alt mr-2"></i> Dapatkan Lokasi Saya
                    </button>
                    <p id="locationStatus" class="mt-2 text-sm text-gray-600"></p>
                </div>
                <div>
                    <label for="radius_absensi_meter" class="block text-gray-700 text-sm font-bold mb-2">Radius Absensi (meter):</label>
                    <input type="number" id="radius_absensi_meter" name="radius_absensi_meter" value="<?php echo htmlspecialchars($settings['radius_absensi_meter'] ?? 50); ?>" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    <p class="text-gray-600 text-xs italic mt-1">Jarak maksimal dari titik koordinat yang diizinkan untuk absensi.</p>
                </div>
            </div>

            <div class="mb-6">
                <h3 class="text-lg font-semibold mb-2">QR Code Absensi</h3>
                <?php if (!empty($settings['qr_code_path'])): ?>
                    <img src="<?php echo htmlspecialchars(ROOT_URL . $settings['qr_code_path']); ?>" alt="QR Code Absensi" class="h-48 w-48 object-contain rounded-lg shadow-sm mb-4">
                <?php else: ?>
                    <p class="text-gray-600 mb-2">Belum ada QR Code yang dibuat.</p>
                <?php endif; ?>
                <a href="<?php echo ROOT_URL; ?>admin/settings/qr_code_generator.php" class="inline-block bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline">
                    <i class="fas fa-qrcode mr-2"></i> Buat/Perbarui QR Code
                </a>
                <p class="text-gray-600 text-xs italic mt-1">QR Code ini harus ditempel di lokasi yang sudah ditentukan.</p>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const latInput = document.getElementById('lokasi_sekolah_lat');
    const longInput = document.getElementById('lokasi_sekolah_long');
    const locationStatus = document.getElementById('locationStatus'); // Ambil elemen pesan

    if (getLocationBtn && latInput && longInput && locationStatus) {
        getLocationBtn.addEventListener('click', function() {
            if ("geolocation" in navigator) {
                // Tampilkan pesan loading
                locationStatus.textContent = "Mencari lokasi Anda... Pastikan GPS aktif dan Anda berada di tempat terbuka.";
                    ...
                    function(error) {
                        ...
                        switch(error.code) {
                            case error.PERMISSION_DENIED:
                                errorMessage = "Akses lokasi ditolak. Izinkan lokasi di pengaturan browser Anda.";
                                break;
                            case error.POSITION_UNAVAILABLE:
                                errorMessage = "Lokasi tidak tersedia. Coba aktifkan GPS.";
                                break;
                            case error.TIMEOUT:
                                errorMessage = "Waktu habis. Gagal mendapatkan lokasi. Coba lagi.";
                                break;
