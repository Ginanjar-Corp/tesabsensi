...
?>
    ...
    <?php if ($is_guru_checked_in && $is_guru_checked_out): ?>
        <p class="text-green-600 font-bold text-lg"><i class="fas fa-check-circle mr-2"></i> Anda sudah absen masuk dan pulang hari ini.</p>
        <p class="text-gray-600 mt-2">Absensi hari ini sudah lengkap. Terima kasih.</p>
    <?php elseif ($is_guru_checked_in): ?>
        ...
        <div class="bg-blue-600 text-white rounded-xl shadow-lg p-6 mb-8 text-center">
            <h1 class="text-3xl font-bold mb-2">Absensi Pulang via QR Code</h1>
            <p class="text-gray-100">Arahkan kamera ke QR Code pulang yang tersedia di pintu keluar sekolah.</p>
    ...
    <?php else: ?>
        ...
        <p class="text-gray-600 mt-2 mb-4">Mohon lakukan absensi masuk Anda terlebih dahulu.</p>

        <div class="bg-blue-600 text-white rounded-xl shadow-lg p-6 mb-8 text-center">
            <h1 class="text-3xl font-bold mb-2">Absensi Masuk via QR Code</h1>
            <p class="text-gray-100">Arahkan kamera ke QR Code yang tersedia di pintu masuk sekolah.</p>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function startQrScanner(type) {
            ...
            if (startQrButton) {
                startQrButton.classList.add('hidden');
            }
            if (startQrCheckoutButton) {
                startQrCheckoutButton.classList.add('hidden');
            }
            ...
            .catch(err => {
                ...
                startQrButton.classList.remove('hidden');
                if (startQrCheckoutButton) {
                    startQrCheckoutButton.classList.remove('hidden');
                }
        ...
        async function sendCheckInRequest(token) {
            showMessage('Memproses absensi masuk...', 'bg-blue-500');
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    async (position) => {
                        ...
                        try {
                            const response = await fetch('<?php echo ROOT_URL; ?>api/guru_check_in.php', {
                                method: 'POST',
                                body: formData
                            });
        ...
        async function sendCheckOutRequest(token) {
            showMessage('Memproses absensi pulang...', 'bg-blue-500');
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    async (position) => {
                        ...
                        try {
                            const response = await fetch('<?php echo ROOT_URL; ?>api/guru_check_out.php', {
                                method: 'POST',
                                body: formData
                            });
                            const result = await response.json();
