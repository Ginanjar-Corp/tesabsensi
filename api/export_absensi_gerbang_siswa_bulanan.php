<?php
// project-root/api/export_absensi_gerbang_siswa_bulanan.php
...
try {
    ...
    // --- 4. Ambil semua siswa yang relevan (berdasarkan filter kelas) ---
    $all_students_in_filter = $siswa_model->getAllStudentsByKelas($filter_kelas_id);

    // --- 5. Ambil semua catatan absensi untuk bulan, tahun ajaran, semester, dan kelas yang difilter ---
    $raw_attendance_data = $absensi_gerbang_model->getMonthlyStudentGateAttendanceReport(
        $selected_month,
        $selected_year,
        $filter_tahun_ajaran,
        $filter_semester,
    ...
    // --- 6. Pivot data absensi per siswa dan per tanggal ---
    $pivoted_data = [];
    foreach ($all_students_in_filter as $student) {
        $pivoted_data[$student['id']] = [
            'nisn' => $student['nisn'],
    ...
    foreach ($raw_attendance_data as $record) {
        ...
        if (isset($pivoted_data[$siswa_id]) && in_array($tanggal, $dates_in_month_to_report)) {
            $status = $record['status_masuk'] ?? 'Alpha';
            $pivoted_data[$siswa_id]['daily_records'][$tanggal] = [
                'status_masuk' => $status
            ];
            // Hitung total kehadiran per siswa
            if ($status === 'Hadir' || $status === 'Terlambat') {
                $pivoted_data[$siswa_id]['counts']['H']++;
            } elseif ($status === 'Sakit') {
                $pivoted_data[$siswa_id]['counts']['S']++;
            } elseif ($status === 'Izin') {
                $pivoted_data[$siswa_id]['counts']['I']++;
            } elseif ($status === 'Alpha') {
                $pivoted_data[$siswa_id]['counts']['A']++;
            }
        }
    }
    ...
    $sheet->setTitle('Absensi Bulanan');
    ...
    $total_columns = 4 + count($dates_in_month_to_report) + 4; // Tambahan 4 kolom untuk total H, S, I, A
    ...
    $sheet->setCellValue('A2', 'Laporan Absensi Gerbang Siswa Bulanan');
    $sheet->mergeCells('A2:' . $end_col_letter . '2');
    $sheet->getStyle('A2')->applyFromArray($mainHeaderStyle);

    // Baris 3: Detail Bulan, Tahun, Semester, Kelas, Tahun Ajaran
    $sheet->setCellValue('A3', 'Bulan: ' . format_date_indonesian("{$selected_year}-{$selected_month}-01") . ' | Semester: ' . ($filter_semester ?? 'Semua') . ' | Kelas: ' . $kelas_name . ' | Tahun Ajaran: ' . ($filter_tahun_ajaran ?? 'Semua'));
    $sheet->mergeCells('A3:' . $end_col_letter . '3');
    $sheet->getStyle('A3')->applyFromArray($subHeaderStyle);

    // Baris 4: Header Kolom
    $column_headers = ['No.', 'NISN', 'Nama Siswa', 'Kelas'];
    ...
    $no = 1;
    foreach ($pivoted_data as $siswa_id => $data) {
        $current_col = 'A';
        $sheet->setCellValue($current_col++ . $row, $no++);
        $sheet->setCellValue($current_col++ . $row, htmlspecialchars($data['nisn'] ?? '-'));
        $sheet->setCellValue($current_col++ . $row, htmlspecialchars($data['nama_lengkap']));
        $sheet->setCellValue($current_col++ . $row, htmlspecialchars($data['nama_kelas']));
        ...
        foreach ($dates_in_month_to_report as $date_key) {
            ...
            $current_cell = $current_col++ . $row;
            $sheet->setCellValue($current_cell, $display_status);
        }
        
        // Tambahkan kolom total per siswa
        ...
        // Tambahkan total per siswa ke grand total
    ...
    $start_col_summary = Coordinate::stringFromColumnIndex(count($dates_in_month_to_report) + 5);
    $sheet->setCellValue($start_col_summary . $row, 'Total');
    $sheet->mergeCells('A' . $row . ':' . Coordinate::stringFromColumnIndex(count($dates_in_month_to_report) + 4) . $row);
    ...
    $filename = 'Laporan_Absensi_Gerbang_Siswa_Bulanan_' . $selected_year . '_' . $selected_month;
    if ($filter_kelas_id) {
        $filename .= '_Kelas_' . str_replace(['/', ' '], ['-', '_'], $kelas_name);
    }
    ...
    error_log("Export Absensi Gerbang Monthly Error: " . $e->getMessage());
    echo "Terjadi kesalahan database saat membuat laporan: " . $e->getMessage();
} catch (Exception $e) {
    error_log("Export Absensi Gerbang Monthly General Error: " . $e->getMessage());
