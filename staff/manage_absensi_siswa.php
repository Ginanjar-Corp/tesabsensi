...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'record_absensi_batch') {
        ...
        redirect(ROOT_URL . 'staff/manage_absensi_siswa.php?kelas_id=' . urlencode($selected_kelas_id ?? '') . '&tanggal=' . urlencode($tanggal_absensi) . '&tahun_ajaran=' . urlencode($selected_tahun_ajaran) . '&semester=' . urlencode($selected_semester));
    }
