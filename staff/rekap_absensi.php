...
if (isset($_POST['action']) && $_POST['action'] === 'view_report') {
    if (!empty($selected_kelas_id)) {
        ...
        redirect(ROOT_URL . 'staff/rekap_absensi.php');
    }
}
...
if (isset($_POST['action']) && $_POST['action'] === 'send_weekly_summary') {
    ...
    redirect(ROOT_URL . 'staff/rekap_absensi.php?kelas_id=' . urlencode($selected_kelas_id) . '&start_date=' . urlencode($startDate) . '&end_date=' . urlencode($endDate));
}
