<?php
// project-root/staff/laporan_absensi_gerbang_siswa.php
...
require_once __DIR__ . '/../classes/Kelas.php'; // Tambahkan kelas Kelas
...
$all_kelas = []; // Untuk dropdown kelas
...
try {
    ...
    $all_kelas = $kelas_model->getAll(); // Asumsi ada metode getAll() di kelas Kelas
    ...
    error_log("Laporan Absensi Gerbang Siswa Error: " . $e->getMessage());
    set_flash('error', 'Terjadi kesalahan database: ' . $e->getMessage());
} catch (Exception $e) {
    error_log("Laporan Absensi Gerbang Siswa General Error: " . $e->getMessage());
...
<div class="container mx-auto p-4">
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Laporan Absensi Gerbang Siswa</h1>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mb-6">
        <h2 class="text-xl font-semibold mb-4">Laporan Harian - Pilih Tanggal, Tahun Ajaran, Semester & Kelas</h2>
        <form id="dailyFilterForm" action="" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div>
                ...
                <select id="kelas_id" name="kelas_id" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    ...
                    <?php foreach ($all_kelas as $kelas): ?>
                        <option value="<?php echo htmlspecialchars($kelas['id']); ?>"
                                <?php echo ($filter_kelas_id == $kelas['id']) ? 'selected' : ''; ?>>
                            <?php echo htmlspecialchars($kelas['nama_kelas']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="flex justify-start items-end col-span-1 md:col-span-2">
                <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline w-full md:w-auto mr-2">
                    Tampilkan Laporan Harian
                </button>
                <button type="button" id="exportDailyExcelBtn" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline w-full md:w-auto">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mb-6">
        <h2 class="text-xl font-semibold mb-4">Laporan Bulanan - Pilih Bulan, Tahun, Tahun Ajaran, Semester & Kelas</h2>
        <form id="monthlyFilterForm" action="" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-xl font-semibold mb-4">Absensi Gerbang Siswa Tanggal: <?php echo htmlspecialchars(format_date_indonesian($selected_date)); ?></h2>
        
        <?php if (empty($report_data)): ?>
            <p class="text-gray-600">Tidak ada data absensi gerbang siswa untuk tanggal ini.</p>
        <?php else: ?>
...
document.getElementById('exportDailyExcelBtn').addEventListener('click', function() {
    ...
    // Arahkan ke endpoint ekspor harian
    window.location.href = '<?php echo ROOT_URL; ?>api/export_absensi_gerbang_siswa.php?' + params.toString();
});
...
document.getElementById('exportMonthlyExcelBtn').addEventListener('click', function() {
    ...
    // Arahkan ke endpoint ekspor bulanan
    window.location.href = '<?php echo ROOT_URL; ?>api/export_absensi_gerbang_siswa_bulanan.php?' + params.toString();
});
</script>
